	...
	private void train() throws Exception {
		...
		for(int steps=0;steps<40000;steps++)
		{
			...
			for (int i=0;i<600;i++){
				...
				layer.calculateActivations(MnistDatabase.trainImages.get(s).data);
				for (int j2 = 0; j2 < targets.length; j2++) {
					targets[j2]=layer.neurons[j2].y;
				}
				layer2.calculateActivations(MnistDatabase.trainImages.get(s2).data);
				for (int j2 = 0; j2 < targets2.length; j2++) {
					targets2[j2]=layer2.neurons[j2].y;
				}
				layer.updateErrors(MnistDatabase.trainImages.get(s).data, targets2);
				layer2.updateErrors(MnistDatabase.trainImages.get(s2).data, targets);
			}
